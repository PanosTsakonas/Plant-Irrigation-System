...
This is the newest version of the code that goes away from using the esp_depp_sleep() function. In my testing of the system i found out that sometimes the deep sleep functionality on the NodeMCU 32s bugs out and stops working so you have to unplug and plug it back in. With this new code I have also created a Web Server were you can manually start the irrigation process. Before I go into more depth about the web server let me just tell you a TLDR of the changes. So the system will check the time and if it is 17:00 it will start watering the plants as usual. Then it will stop doing that until the next day at 17:00. This is ensured by the l=1 variable that changes back to 0 at 16:00. I have also added a WiFi connect check at the end of the void lopp function because the esp_deep_sleep is out. In case the WiFI goes down, for some reason, it will wait for it to come back. And because the system doesn't have a counter it doesn't really matter what time the WiFi connects back up! The rest of the code for that is similar to my previous examples. For the Web Server now you have to connect to the Irrigation System SSID that you can find on your WiFi list. The password can change in the code but for this example is 12345678. Once connected open a web browser. In the search field type 192.168.1.1. It will then send you to a page that it will have a button and it will display the soil humidity percentage. Upon clicking once the button the irrigation process starts and it will display the temperature and air humidity on the page. Upon clicking the same button again it will stop the irrigation process and it will display the soil humidity at the time you pressed the button.
